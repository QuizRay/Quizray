-- ==============================================================================
-- QuizRay Database Migration: Phase 3B-2 Bug Fix
-- Fix Category Color Schema Mismatch in get_user_performance_analytics RPC
-- ==============================================================================

-- Correct public.get_user_performance_analytics() to remove references to
-- the non-existent public.categories.color column while preserving the
-- 'category_color' field (defaulting to 'blue') for client interface compatibility.
...
BEGIN
  ...
  -- 3. Category Performance (Grouped by existing columns: id, name, icon)
  SELECT COALESCE(
    jsonb_agg(
      jsonb_build_object(
